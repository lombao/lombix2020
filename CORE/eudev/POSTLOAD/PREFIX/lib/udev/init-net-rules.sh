...
RULES=%PREFIX%/etc/udev/rules.d/70-persistent-net.rules 
...
function ignore_if
{
   ...
   if echo $byte2 | /usr/bin/grep -q "[2367abef]"; then return 0; fi
...
function comment
{
  ...
  # Default
  driver=$(/usr/bin/basename $(/usr/bin/readlink -f $NIC/device/driver/module))
  export COMMENT="net device ${driver}"
}

if ! /usr/bin/mountpoint -q /sys; then
  msg="/sys mut be mounted"
  usage
fi

if ! /usr/bin/mountpoint -q /proc; then
...
for NIC in $DEVICES; do
     IF=${NIC##*/}
     if echo $NIC | /usr/bin/grep -q '*' ; then continue; fi
     ...
     %PREFIX%/lib/udev/write_net_rules 
done
